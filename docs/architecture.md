...
Cette page décrit la structure technique actuelle de Papers Empire. Elle résume les modules clés, les flux de données et présente deux diagrammes de séquence PlantUML.
...
database "localStorage" as Store
...
actor Player
participant "app.js" as App
participant "Achievements" as Ach
collections "achievementsState" as AchState

Player -> App: action (click/achat)
App -> Ach: evaluate(gameState, unlocked)
Ach -> App: renvoie IDs débloqués
